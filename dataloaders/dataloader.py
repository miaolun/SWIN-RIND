...
def Trans(x):
    input_image = np.array(x).astype(np.float32)
    ...
    input = image.permute(2, 0, 1)
    # input[0, :, :] = (input[0, :, :] - 123.68) * 0.017
    # input[1, :, :] = (input[1, :, :] - 116.78) * 0.017
    # input[2, :, :] = (input[2, :, :] - 103.94) * 0.017
    return input
